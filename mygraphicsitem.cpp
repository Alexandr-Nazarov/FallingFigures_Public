  ...
  //---вертикальное движение
    ...
    //---горизонтальное движение
         //общие законы
          ...
          if ((m_y+m_height)>=(m_frame_bottom) && m_falling){                     //торможение при касании нижней границы
              m_v_horr=m_v_horr*m_constants.c_e;
          }
          if ((m_y)<=(m_frame_height) && !m_falling){                     //торможение при касании нижней границы
              m_v_horr=m_v_horr*m_constants.c_e;
          }
...
//         }
       ...
       //вспомогательный расчет
        ...
        if (this->center().x()<=other->center().x()){
             angle_conc=std::asin((center().y()-intersection.y())/ b_ocr)-M_PI/2; // (1)  //!! //угол касания //теорема синусов// в радианах все
          ...
          //   angle_conc=/*M_PI/2-*/std::asin((intersection.y()-center().y())/ b_ocr);/*+M_PI/2; *///!!
           //    angle_conc=/*M_PI/2-*/std::asin((-intersection.y()+center().y())/ b_ocr);      // (2)
            ...
            angle_conc=-(std::asin((center().y()-intersection.y())/b_ocr)+M_PI/2);             // (2)  //!!
         }
        } else { angle_conc=0;}
